sse.add("messagesBadge", function (data) {
    if (USER.setor !== 0) {
...
function setMyPosition() {
    if (navigator.onLine && navigator.geolocation && parseInt(USER.id) > 0) {
        navigator.geolocation.watchPosition(position => {
            setUserData({lat: position.coords.latitude, lng: position.coords.longitude});
        }, erro => {
            console.log(erro);
