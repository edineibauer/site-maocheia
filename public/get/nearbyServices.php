...
$sql->exeCommand("SELECT CP.*, P.*, (6371 * acos(cos(radians({$latitude})) * cos(radians(latitude)) * cos(radians({$longitude}) - radians(longitude)) + sin(radians({$latitude})) * sin(radians(latitude)))) AS distance FROM mao_coordenadas_profissional as CP INNER JOIN mao_profissional as P ON CP.profissional = P.id HAVING distance <= " . $dist);
if($sql->getResult()) {
